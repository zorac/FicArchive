...
###############################################################################
    ...
    $page_title = 'Submit a Story - Story';
    include('header.php');

    echo "<h2>Submit a Story</h2>\n";

    if ($submit['story']) {
        submit_links('author', 'series');
    } elseif ($submit['author']) {
        submit_links('author', '');
    } else {
        fa_error('', 'You have not reached this point in the submission ' .
            'process.');
    }
?>
<h3>Step 2: Select a Story</h3>
<table class="info">
<?php
    $result = mysql_query('
